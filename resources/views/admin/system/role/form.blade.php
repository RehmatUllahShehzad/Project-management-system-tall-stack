<div class="col-span-12 space-y-4">
    <x-admin.components.card heading="Basic Information">
        <div class="grid grid-cols-2 gap-4">
            <x-admin.components.input.group label="{{ __('inputs.name') }}" for="name" :error="$errors->first('role.name')">
                <x-admin.components.input.text wire:model.defer="role.name" name="name" id="name" :error="$errors->first('role.name')" />
    ...
    <div class="px-4 py-3 text-right rounded shadow bg-gray-50 sm:px-6">
        <button type="submit" class="inline-flex justify-center px-4 py-2 text-sm font-medium text-white bg-indigo-600 border border-transparent rounded-md shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
            {{ __($role->id ? 'roles.form.update_btn' : 'roles.form.create_btn') }}
        </button>
    </div>
            ...
            <div class="p-6 space-y-4 text-sm">
                <div class="grid grid-cols-12 gap-4">
                    <div class="col-span-12 md:col-span-6">
                        <strong>{{ __('roles.form.danger_zone.label') }}</strong>
                        <p class="text-xs text-gray-600">{{ __('roles.form.danger_zone.instructions') }}</p>
                    </div>
